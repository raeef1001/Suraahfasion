...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	function SetHTMLHeader($header = '', $OE = '', $write = false)
	{
		...
		if ($this->setAutoTopMargin == 'pad') {
			...
			$sgb_basic_stng = get_option('rtwwcpig_body_top_margin');

			if( $sgb_basic_stng != '' )
			{
				$sgb_top_marg = $sgb_basic_stng;	
			} 
			else 
			{
				$sgb_top_marg = 16;
	...
	function Footer()
	{
		...
		if (($this->watermarkText) && ($this->showWatermarkText)) {
			$rtwwcpig_setting = get_option('rtwwcpig_watermark_rotation');
			if( $rtwwcpig_setting != '' ){
		  		$rotate_custom = $rtwwcpig_setting;
		  	}else{
		  		$rotate_custom = 45;
		  	}
