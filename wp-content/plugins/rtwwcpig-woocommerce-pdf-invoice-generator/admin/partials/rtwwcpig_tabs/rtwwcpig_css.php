...
if(!empty($rtwwcpig_purchase_code_details) && is_array($rtwwcpig_purchase_code_details) && !empty($rtwwcpig_purchase_code_details) && isset($rtwwcpig_purchase_code_details['status']) &&  $rtwwcpig_purchase_code_details['status'] == true && isset($rtwwcpig_purchase_code_details['purchase_code']) && $rtwwcpig_purchase_code_details['purchase_code'] != '' )
{
	...
	if(isset($_POST['rtwwcpig_submit'])) {
		...
		<div class="notice notice-success is-dismissible">
			...
			woocommerce_update_options( rtwwcpig_css_settings() );

			update_option( 'rtwwcpig_css_setting_opt', sanitize_post($_POST['rtwwcpig_css_setting_opt']) );
		}

		settings_fields('rtwwcpig_css_setting');
		$rtwwcpig_get_setting = get_option('rtwwcpig_css_setting_opt');
	...
	<div class="rtwwcpig_tab_div">
		<?php woocommerce_admin_fields(rtwwcpig_css_settings()); ?> 
	</div>
	...
	<table class="wp-list-table form-table rtw-table">
		<tbody>
			<tr>
				...
				<td class="descr_2">
					<select name="rtwwcpig_css_setting_opt[rtwwcpig_pdf_page_size]">
 ...
 * @since    1.0.0
 */
function rtwwcpig_css_settings()
{
	...
	$settings =	array(
		'section_title' => array(
			...
			'type'     => 'title',
			'desc'     => '',
			'id'       => 'rtwwcpig_css_setting_opt'
		),
		array(
			'id'          => 'rtwwcpig_page_orien',
			'option_key'  => 'rtwwcpig_page_orien',
		...
		array(
			'id'          => 'rtwwcpig_body_font_family',
			'option_key'  => 'rtwwcpig_body_font_family',
